...
async function renderValueAddPdf(valueAddId, host, cookieHeader = '') {
  ...
  // 4) wait for a solid indicator (10 s max)
  try {
    await page.waitForSelector('.value-add-page, .report-wrapper', { timeout: 10000 });
  } catch {
    if (DEBUG_SURGE) console.warn(`[VA ${valueAddId}] selector timeout (still continuing)`);
  }

  // 5) dump screenshot + html if debugging
  if (DEBUG_SURGE) {
...
/* ──────────────────────────────────────────────────────────────────────────
 * Build, upload, and persist ONE household’s packet.
 * Now preserves the exact composer order stored in `surge.order`.
 *
 * @param {Object}   surge        Surge Mongoose doc (populated)
 * @param {String}   householdId  Household ObjectId string
 * @param {String}   host         e.g. "https://app.surgetk.com"
 * @param {Function} progressCb   (inc:number=1) => void   – granular progress
 * @param {Function} emitFn       Legacy socket emit (“done” only)
 * @param {String}   cookieHeader Raw Cookie header for auth in Puppeteer
 * @returns {Boolean}             true = success (for queue accounting)
 * ------------------------------------------------------------------------ */
